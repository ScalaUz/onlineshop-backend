...
CREATE TYPE ORDER_STATUS AS ENUM ('unpaid', 'in_progress', 'canceled', 'delivered');
CREATE TYPE PAYMENT_METHOD AS ENUM ('payme', 'click_up');
...
CREATE TABLE IF NOT EXISTS categories (
  ...
  parent_id UUID NULL CONSTRAINT fk_categories REFERENCES categories (id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS customers_addresses(
  customers_id UUID NOT NULL CONSTRAINT fk_users REFERENCES customers (id) ON UPDATE CASCADE ON DELETE CASCADE UNIQUE,
  country_id UUID NOT NULL CONSTRAINT fk_countries REFERENCES countries (id) ON UPDATE CASCADE ON DELETE CASCADE,
  region_id UUID NOT NULL CONSTRAINT fk_regions REFERENCES regions (id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS orders(
  id UUID PRIMARY KEY NOT NULL,
  customer_id UUID NOT NULL CONSTRAINT fk_customers REFERENCES customers (id) ON UPDATE CASCADE ON DELETE CASCADE,
...
create table if not exists order_items(
  id UUID PRIMARY KEY NOT NULL,
  order_id UUID NOT NULL CONSTRAINT fk_orders REFERENCES orders (id) ON UPDATE CASCADE ON DELETE CASCADE,
  product_id UUID NOT NULL CONSTRAINT fk_products REFERENCES products (id) ON UPDATE CASCADE ON DELETE CASCADE,
...
INSERT INTO
  users (id, created_at, name, role, email, password)
VALUES
  (
